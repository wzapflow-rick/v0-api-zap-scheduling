...
describe('normalizeBusinessConfig', () => {
  it('retorna fallback seguro quando a entrada é inválida', () => {
    ...
    expect(config.labels.client.singular).toBe('Cliente');
  });
...
describe('DEFAULT_BUSINESS_CONFIG', () => {
  it('usa a versão de formato atual e habilita os módulos principais', () => {
    expect(DEFAULT_BUSINESS_CONFIG.version).toBe(CURRENT_CONFIG_VERSION);
    expect(DEFAULT_BUSINESS_CONFIG.features.medicalRecords.enabled).toBe(true);
  });
});
